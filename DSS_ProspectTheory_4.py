...
# =====================================================================
# 2. MATHEMATISCHE EVALUATIONSFUNKTIONEN (ORIGINALE PT MIT GEWICHTEN)
# =====================================================================
def pi_weight(p):
    """Subadditive Wahrscheinlichkeitsgewichtung der originalen PT (1979)."""
    if np.isclose(p, 0.4): return 0.12  # Mensch unterschätzt 40% massiv
    if np.isclose(p, 0.1): return 0.04  # Mensch unterschätzt 10% massiv
    if np.isclose(p, 0.5): return 0.45  # 50% wird relativ höher bewertet
    return p

def prospect_value(x):
    """Standard-Wertfunktion mit den K&T-Standardparametern."""
    alpha = 0.88
    lmbda = 2.25
    gains_clean = np.maximum(x, 0) ** alpha
    losses_clean = -lmbda * (np.abs(np.minimum(x, 0)) ** alpha)
    return np.where(x >= 0, gains_clean, losses_clean)

# Berechnung der normativen Kennzahlen (EV bleibt linear)
ev_a = np.sum(outcomes_a * probs_a)
ev_b = np.sum(outcomes_b * probs_b)

# HIER LIEGT DIE ANPASSUNG: Anwendung von pi_weight auf die Wahrscheinlichkeiten
v_a = np.sum(prospect_value(outcomes_a) * np.array([pi_weight(p) for p in probs_a]))
v_b = np.sum(prospect_value(outcomes_b) * np.array([pi_weight(p) for p in probs_b]))
...
if min_ev >= 0 and min_v >= 0:
    ax2.set_ylim(0, max(140, max_ev * 1.2))
    ax2_twin.set_ylim(0, max(12, max_v * 1.2))
...
plt.suptitle(f"IT-Decision Support System (Prospect Theory Analysis)\nAlpha={ALPHA} | Beta={BETA} | Lambda={LAMBDA}", fontsize=13, fontweight='bold')
plt.tight_layout()
plt.show()