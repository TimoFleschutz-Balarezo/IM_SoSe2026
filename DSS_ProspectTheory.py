...
# =====================================================================
def prospect_value(x, alpha=ALPHA, beta=BETA, lmbda=LAMBDA):
    """Berechnet den subjektiven Wert v(x) nach Kahneman & Tversky."""
    gains_clean = np.maximum(x, 0) ** alpha
    losses_clean = -lmbda * (np.abs(np.minimum(x, 0)) ** beta)
